...
int main()
{
    ...
    for (size_t i = 0; i < imagList.size(); i++)
    {
        ...
        detector_yolov7plate.Detect_Yolov7Plate(ImgVec,dets);
        ...
        for (int k = 0; k < dets.size(); k++)
        {
            for (size_t f = 0; f < dets[k].size(); f++)
            {
                ...
                cv::putText(ImgVec[k], label3, cv::Point(dets[k][f].bbox.xmin, dets[k][f].bbox.ymin), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255, 255, 0),1);
            }
    ...
    detector_yolov7plate.ReleaseDetector_Yolov7Plate();
    return 0;
}